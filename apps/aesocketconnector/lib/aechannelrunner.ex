defmodule AeChannelRunner do
  ...
  def start_channel_helper() do
    start_channel(
      AeTestAccounts.initiatorPubkey(),
      AeTestAccounts.initiatorPrivkey(),
      AeTestAccounts.responderPubkey(),
      AeTestAccounts.responderPrivkey(),
      @ae_url,
      @network_id
    )
  end
  ...
  def start_channel(
    ...
    state_channel_configuration = %AeSocketConnector.WsConnection{
    ...
    AeSessionHolder.run_action(pid_initiator, fn pid ->
      AeSocketConnector.new_contract(pid, "contracts/TicTacToe.aes")
    end)
    ...
    AeSessionHolder.run_action(pid_responder, fn pid ->
      AeSocketConnector.withdraw(pid, 1_000_000)
    end)
    ...
    AeSessionHolder.run_action(pid_responder, fn pid ->
      AeSocketConnector.deposit(pid, 1_000_000)
    end)
